...
export class AuthController {
  ...
  async login(@Body() payload: LoginRequest, @Res() res: any) {
    try {
      ...
      return res.status(HttpStatus.OK).json({ token: _token, success: "Login Successfully" });
    } catch (error) {
      return res.status(HttpStatus.SERVER_ERROR).json({
        success: false, message: "Invalid login credentials: " + error,
      });
    }
  }

  @Post("/register")
  async register(@Body() payload: UserRequest, @Res() res: any) {
    try {
